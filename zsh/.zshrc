...
# macOS Terminal.app should start blank — no fetch, no p10k frame, no instant
# prompt. Fedora/Hyprland keep the logo + two-line prompt.
ff() {
...
if [[ "$(uname -s)" == Darwin ]]; then
  # One character at the top of a new window. No frame, no extra newline, no rprompt.
  PROMPT='❯ '
  RPROMPT=
else
