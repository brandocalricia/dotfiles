#!/usr/bin/env python3
"""
BrandoObsid-doctor — audit and repair the BrandoObsid vault at ~/Documents/BrandoObsid.
...
VAULT = Path(os.environ.get("BRANDOOBSID_VAULT", Path.home() / "Documents" / "BrandoObsid"))
ARCHIVE = Path(os.environ.get("BRANDOOBSID_ARCHIVE", Path.home() / "Documents" / "BrandoObsid-archive"))
...
def snapshot() -> Path | None:
    ...
    """
    snap_dir = Path.home() / ".local" / "share" / "BrandoObsid-snapshots"
    snap_dir.mkdir(parents=True, exist_ok=True)
    stamp = datetime.now().strftime("%Y-%m-%dT%H%M%S")
    dest = snap_dir / f"BrandoObsid-{stamp}.tar.gz"
    import subprocess
    r = subprocess.run(
    ...
    if r.returncode != 0:
        return None
    old = sorted(snap_dir.glob("BrandoObsid-*.tar.gz"))[:-10]
    for f in old:
...
def write_report(a: dict, sc: int, deductions: list[str], actions: list[str]) -> None:
    ...
    L = [
        ...
        "",
        "> Generated by `BrandoObsid-doctor.py`. Do not hand-edit — it is overwritten.",
        "",
    ]
    if deductions:
...
# --------------------------------------------------------------------------
def main() -> int:
    ap = argparse.ArgumentParser(description="Audit and repair the BrandoObsid vault.")
    ...
    if not VAULT.is_dir():
        print(f"BrandoObsid-doctor: vault not found at {VAULT}", file=sys.stderr)
        return 1
