...
- 󰖟 Brave (nf-md-web globe — inherits bar cream; do not use the color logo font)
- 󰦝 NordVPN (shield+lock so it reads as VPN at bar size; same cream)
- 󰌌 Monkeytype (Brave site window or title)
Color sbix logos in `waybar/fonts/` are unused: CSS `color` cannot tint them,
so they never matched workspaces across themes.
